...
class SignalGeneratorApp(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        self.slider_am = ctk.CTkSlider(
            frm_am, from_=0, to=1, number_of_steps=100
        )
        ...
        self.slider_fm = ctk.CTkSlider(
            frm_fm, from_=0, to=500, number_of_steps=100
        )
    ...
    def plot_signal(self):
        ...
        if self.mod_fm.get():
            ...
            y = np.sin(2 * np.pi * Fc * t + d * np.sin(2 * np.pi * (Fc / 4) * t))

        # FFT
        Y = fftshift(fft(y))
        f = fftshift(fftfreq(N, 1 / Fs))

        # Armazenar para zoom/export
