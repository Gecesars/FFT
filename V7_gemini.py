...
class SignalGeneratorApp(ctk.CTk):
    ...
    def _build_status_bar(self):
        self.status_bar = ctk.CTkLabel(
            ...
            text="Pronto",
            anchor="w",
            fg_color="#222222",  # Fundo escuro
            text_color="white",  # Texto branco para contraste
            font=("Arial", 11, "italic")
        )
        self.status_bar.pack(side="bottom", fill="x", padx=0, pady=(0, 0))
    ...
    def _on_mouse_motion(self, event):
        if self.dragging_marker and event.inaxes and event.xdata and event.ydata:
            # Atualiza marcador vertical
            if hasattr(self.dragging_marker, 'get_xdata'):
                self.dragging_marker.set_xdata([event.xdata])
            # Atualiza marcador horizontal
            if hasattr(self.dragging_marker, 'get_ydata'):
    ...
    def add_marker(self, kind):
        ...
        # Verifica se está dentro da janela visível
        if kind.endswith('_v') and not (ax.get_xlim()[0] <= ev.xdata <= ax.get_xlim()[1]):
            return
        elif kind.endswith('_h') and not (ax.get_ylim()[0] <= ev.ydata <= ax.get_ylim()[1]):
            return

        # Limita a 2 marcadores por tipo
        if len(self.markers[kind]) >= 2:
            msg = "Máximo de 2 marcadores por tipo atingido."
            self.status_bar.configure(text=msg)
            return

        autoscale_state = ax.get_autoscalex_on(), ax.get_autoscaley_on()
        ax.set_autoscalex_on(False)
        ax.set_autoscaley_on(False)

        if kind.endswith('_v'):
            line = ax.axvline(x=ev.xdata, linestyle='--',
                              color='red' if kind.startswith('time') else 'lime', picker=5)
        else:
            line = ax.axhline(y=ev.ydata, linestyle='--',
                              color='red' if kind.startswith('time') else 'lime', picker=5)

        ax.set_autoscalex_on(autoscale_state[0])
        ax.set_autoscaley_on(autoscale_state[1])
