...
class SignalGeneratorApp(ctk.CTk):
    ...
    def _update_plots(self):
        ...
        # Salva os limites atuais antes de limpar
        prev_time_xlim = self.ax_time.get_xlim() if self.ax_time.lines else None
        prev_freq_xlim = self.ax_freq.get_xlim() if self.ax_freq.lines else None

        # Limpa apenas os dados, mantendo marcadores
        self.ax_time.clear()
        self.ax_freq.clear()

        # Reaplica os marcadores existentes
        for marker_type in self.markers:
            for marker in self.markers[marker_type]:
                if marker_type == 'time_v':
                    self.ax_time.axvline(marker.get_xdata()[0], color=marker.get_color(), linestyle='--')
                elif marker_type == 'time_h':
                    self.ax_time.axhline(marker.get_ydata()[0], color=marker.get_color(), linestyle='--')
                elif marker_type == 'freq_v':
                    self.ax_freq.axvline(marker.get_xdata()[0], color=marker.get_color(), linestyle='--')
                elif marker_type == 'freq_h':
                    self.ax_freq.axhline(marker.get_ydata()[0], color=marker.get_color(), linestyle='--')
        ...
        # Restaura os limites anteriores ou aplica novos
        if prev_time_xlim:
            self.ax_time.set_xlim(prev_time_xlim)
        else:
            self.ax_time.set_xlim(self.last_data['t'][0], self.last_data['t'][-1])

        if prev_freq_xlim:
            self.ax_freq.set_xlim(prev_freq_xlim)
        else:
            self.ax_freq.set_xlim(self.last_data['f'][0], self.last_data['f'][-1])

        self.fig.tight_layout(pad=3.0)
        self.update_time_zoom(self.zoom_time.get())
        self.update_freq_zoom(self.zoom_freq.get())
        self.canvas.draw()
