...
class SignalGeneratorApp(ctk.CTk):
    def __init__(self):
        ...
        self.imported_voltage_scale = [1.0, "V", 1]
        self.imported_time_scale = [1.0, "S", 1]
        ...
        logger.debug("Aplicação inicializada")
    ...
    def _build_sidebar(self):
        logger.debug("Construindo sidebar...")
        ...
        self.entry_vpp = self._add_entry(frm_gen, "Amplitude (Vpp):", "1.0")
    ...
    def _build_plot_area(self):
        logger.debug("Construindo área de plotagem...")
    ...
    def update_time_scale(self, value):
        ...
        try:
            ...
            logger.error(f"Erro ao atualizar escala de tempo: {str(e)}")
    ...
    def update_freq_scale(self, value):
        ...
        try:
            ...
            logger.error(f"Erro ao atualizar escala de frequência: {str(e)}")

    def _build_context_menu(self):
        logger.debug("Construindo menu de contexto...")
    ...
    def _build_marker_panel(self):
        logger.debug("Construindo painel de marcadores...")
    ...
    def _build_analysis_panels(self):
        logger.debug("Construindo painéis de análise...")
        ...
        # Correção crucial: Criar as abas primeiro
        ...
        # Agora criar os frames dentro das abas
        ...
        for label_text, var in metrics:
            ...
            val = ctk.CTkLabel(frame, textvariable=var, width=120, anchor="e")
        ...
        logger.debug("Painéis de análise construídos")
    ...
    def submit_plot_task(self):
        logger.debug("Submetendo tarefa de plotagem...")
    ...
    def _compute_and_plot_task(self):
        try:
            logger.debug("Iniciando computação e plotagem...")
            params = self._validate_inputs()
            if not params:
                logger.warning("Parâmetros inválidos, abortando computação")
            ...
            logger.debug(f"Dados gerados: t={len(t)}, y={len(y)}, f={len(f)}, Y={len(Y)}")

            self.after(0, self._update_plots)
            self.after(0, self.update_analysis_panels)
            logger.debug("Tarefas de atualização agendadas")

        except Exception as e:
            error_msg = str(e)
            logger.error(f"Erro na computação: {error_msg}\n{traceback.format_exc()}")
    ...
    def _validate_inputs(self):
        try:
            ...
            logger.debug(f"Parâmetros validados: {p}")
            return p
    ...
    def _show_wav_preview(self, t, y, filename):
        ...
        freq_tab = analysis_notebook.add("Análise de Frequência")
    ...
    def _open_sampling_window(self, parent, t, y, filename):
        ...
        sample_values = np.interp(sample_points, t, y) * 1000
    ...
    def _on_sample_motion(self, event):
        ...
        state['sample_values'][idx] = event.ydata / 1000

        if not event.key == 'shift':
            state['sample_points'][idx] = event.xdata

        state['line'].set_data(state['sample_points'], state['sample_values'] * 1000)
        state['canvas'].draw()
    ...
    def _calculate_freq_analysis(self, f, Y):
        ...
        return [
            ...
            ("SNR (Relação Sinal-Ruído):", f"{snr:.2f} dB" if snr != float('inf') else "∞ dB")
        ]

    def import_wav(self):
        logger.debug("Iniciando importação de WAV...")
        filepath = filedialog.askopenfilename(filetypes=[("WAV files", "*.wav")])
        if not filepath:
            logger.debug("Importação cancelada pelo usuário")
            return

        try:
            header, data_buffers = self._read_wav_file(filepath)
            volt_scale, ts = self._parse_header(header)
            self.imported_voltage_scale = volt_scale[0]
            self.imported_time_scale = ts

            ch1_data = self._parse_channel_data(data_buffers[0], volt_scale[0])
            N = len(ch1_data)
            time_per_division = ts[0] * ts[2]
            total_time = 12 * time_per_division
            t = np.linspace(0, total_time, N, endpoint=False)
            Fs = N / total_time
            vpp = max(ch1_data) - min(ch1_data)

            Y = fftshift(fft(ch1_data))
            f = fftshift(fftfreq(N, 1 / Fs))

            self.last_data = {'t': t, 'y': ch1_data, 'f': f, 'Y': np.abs(Y)}
            logger.debug(f"WAV importado: t={len(t)}, y={len(ch1_data)}, f={len(f)}, Y={len(Y)}")
            ...
            self.after(0, lambda: self.entry_fc.insert(0, "0"))
            ...
            logger.debug("Importação de WAV concluída com sucesso")

        except Exception as e:
            error_msg = str(e)
            logger.error(f"Erro na importação de WAV: {error_msg}\n{traceback.format_exc()}")
    ...
    def export_wav(self):
        logger.debug("Iniciando exportação de WAV...")
        if not self.last_data:
    ...
    def _update_plots(self):
        logger.debug("Atualizando plots...")
        if not self.last_data:
        ...
        try:
            ...
            logger.debug("Plots atualizados com sucesso")
        except Exception as e:
            logger.error(f"Erro ao atualizar plots: {str(e)}\n{traceback.format_exc()}")
    ...
    def _adjust_initial_view(self):
        ...
        try:
            ...
            max_freq = max(5 * fundamental_freq, 100)
    ...
    def update_analysis_panels(self):
        logger.debug("Atualizando painéis de análise...")
        if not self.last_data:
            logger.warning("Nenhum dado para análise")
            return

        try:
            logger.debug(f"Últimos dados: t={len(self.last_data.get('t', []))}, "
                         f"y={len(self.last_data.get('y', []))}, "
                         f"f={len(self.last_data.get('f', []))}, "
                         f"Y={len(self.last_data.get('Y', []))}")

            y = np.array(self.last_data['y'])
            t = self.last_data['t']
            f = self.last_data['f']
            Y = self.last_data['Y']

            # Análise no domínio do tempo
            logger.debug("Calculando análise de tempo...")
            if len(y) > 0:
                vpp = np.max(y) - np.min(y)
                logger.debug(f"Vpp calculado: {vpp}")
                self.time_analysis_results['vpp'].set(f"{vpp:.4f} V")

                rms = np.sqrt(np.mean(y ** 2))
                logger.debug(f"RMS calculado: {rms}")
                self.time_analysis_results['rms'].set(f"{rms:.4f} V")

                mean = np.mean(y)
                logger.debug(f"Média calculada: {mean}")
                self.time_analysis_results['mean'].set(f"{mean:.4f} V")

                crest_factor = np.max(np.abs(y)) / rms if rms > 0 else 0
                logger.debug(f"Fator de crista calculado: {crest_factor}")
                self.time_analysis_results['crest_factor'].set(f"{crest_factor:.4f}")

                zero_crossings = np.where(np.diff(np.sign(y)))[0]
                if len(zero_crossings) > 0:
                    zero_crossing_rate = len(zero_crossings) / (t[-1] - t[0])
                    logger.debug(f"Taxa de cruzamento por zero: {zero_crossing_rate}")
                ...
                if len(zero_crossings) > 1:
                    ...
                    if len(periods) > 0:
                        avg_period = np.mean(periods) * 2
                        freq_est = 1 / avg_period if avg_period > 0 else 0
                logger.debug(f"Frequência estimada: {freq_est}")
                self.time_analysis_results['frequency'].set(f"{freq_est:.2f} Hz" if freq_est > 0 else "---")
                ...
                logger.debug(f"Duty cycle: {duty_cycle}")
                self.time_analysis_results['duty_cycle'].set(
                    f"{duty_cycle:.1f}%" if isinstance(duty_cycle, float) else duty_cycle)

                peak_to_rms = np.max(np.abs(y)) / rms if rms > 0 else 0
                logger.debug(f"Relação Pico/RMS: {peak_to_rms}")
                self.time_analysis_results['peak_to_rms'].set(f"{peak_to_rms:.4f}")

                kurt_val = kurtosis(y) if len(y) > 3 else float('nan')
                logger.debug(f"Curtose: {kurt_val}")
                self.time_analysis_results['kurtosis'].set(f"{kurt_val:.4f}" if not np.isnan(kurt_val) else "---")

                skew_val = skew(y) if len(y) > 2 else float('nan')
                logger.debug(f"Assimetria: {skew_val}")
                self.time_analysis_results['skewness'].set(f"{skew_val:.4f}" if not np.isnan(skew_val) else "---")
            else:
                logger.warning("Dados y vazios, análise de tempo não realizada")

            # Análise no domínio da frequência
            logger.debug("Calculando análise de frequência...")
            if len(Y) > 0:
                positive_mask = f > 0
                ...
                logger.debug(f"Frequência fundamental: {fundamental_freq}")
                ...
                logger.debug(f"SNR: {snr} dB, Noise floor: {noise_floor}")
                self.freq_analysis_results['snr'].set(f"{snr:.2f} dB" if isinstance(snr, float) else snr)
                self.freq_analysis_results['noise_floor'].set(
                    f"{noise_floor:.4f}" if isinstance(noise_floor, float) else noise_floor)
                ...
                logger.debug(f"SFDR: {sfdr}")
                self.freq_analysis_results['sfdr'].set(sfdr)
                ...
                logger.debug(f"Largura de banda: {bandwidth}")
                self.freq_analysis_results['bandwidth'].set(
                    f"{self._format_freq(bandwidth)}" if isinstance(bandwidth, float) else bandwidth)
                ...
                logger.debug(f"Índice de modulação: {mod_index}")
                self.freq_analysis_results['mod_index'].set(
                    f"{mod_index:.1f}%" if isinstance(mod_index, float) else mod_index)

                peak_freq = "---"
                if len(harmonic_peaks) > 0:
                    peak_harmonic_idx = harmonic_peaks[0]
                    peak_freq = harmonic_freqs[peak_harmonic_idx]
                logger.debug(f"Frequência de pico: {peak_freq}")
                self.freq_analysis_results['peak_freq'].set(
                    f"{self._format_freq(peak_freq)}" if isinstance(peak_freq, float) else peak_freq)
            else:
                logger.warning("Dados Y vazios, análise de frequência não realizada")

            logger.debug("Painéis de análise atualizados")

            # Forçar atualização visual
            self.analysis_notebook.configure(height=300)
            self.update_idletasks()

        except Exception as e:
            logger.error(f"Erro ao atualizar painéis de análise: {str(e)}\n{traceback.format_exc()}")
...
if __name__ == "__main__":
    logger.debug("Iniciando aplicação principal")
