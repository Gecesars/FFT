...
class SignalGeneratorApp(ctk.CTk):
    ...
    def update_analysis_panels(self):
        ...
        try:
            # Limitar pontos para análise em sinais grandes
            if len(self.last_data['y']) > ANALYSIS_MAX_POINTS:
                self.logger.warning(f"Sinal muito grande ({len(self.last_data['y'])} pontos). Amostrando para análise")

                # Amostrar o sinal para análise de tempo
                step = max(1, len(self.last_data['y']) // ANALYSIS_MAX_POINTS)
                y = np.array(self.last_data['y'][::step])
                t = np.array(self.last_data['t'][::step])

                # Usar FFT já calculada com pontos limitados
                f = self.last_data['f']
                Y = self.last_data['Y']
            else:
            ...
            self.logger.error(error_msg, exc_info=True)
            self.set_status(f"❌ {error_msg}", "red")
            # Adicione logs adicionais para diagnóstico
