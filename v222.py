...
class SignalGeneratorApp(ctk.CTk):
    def __init__(self):
        ...
        # limpar modulações mutuamente exclusivas
    ...
    def _build_sidebar(self):
        ...
        def section(label, color=None):
            frm = ctk.CTkFrame(sidebar, fg_color=color or "#333333", corner_radius=6)
            frm.pack(fill="x", pady=5)
            ctk.CTkLabel(frm, text=label, font=("Arial", 12, "bold")).pack(anchor="w", padx=5, pady=(5,2))
            return frm

        # Geração de Sinal
        frm_gen = section("Geração de Sinal", "#444444")
        self.entry_points = self._add_entry(frm_gen, "Pontos (≥128, múltiplo 2):", "1024")
        self.entry_fc     = self._add_entry(frm_gen, "Fc (Hz):", "100")
        self.entry_fs     = self._add_entry(frm_gen, "Fs (Hz):", "1000")
        ctk.CTkLabel(frm_gen, text="Forma de Onda:").pack(anchor="w", padx=5, pady=(5,0))
        self.waveform = ctk.CTkOptionMenu(frm_gen, values=[
            "Seno","Cosseno","Quadrada","Triangular","Dente de Serra","Pulso","Ruído Branco",
        ...
        chk_am = ctk.CTkCheckBox(frm_am, text="Ativar AM", variable=self.mod_am, command=self._on_am_fm_toggle)
        chk_am.pack(anchor="w", padx=5, pady=(0,5))
        ...
        chk_fm = ctk.CTkCheckBox(frm_fm, text="Ativar FM", variable=self.mod_fm, command=self._on_am_fm_toggle)
        chk_fm.pack(anchor="w", padx=5, pady=(0,5))
        self.slider_fm = ctk.CTkSlider(frm_fm, from_=0, to=500, number_of_steps=100)
        self.slider_fm.set(50)
        self.slider_fm.pack(fill="x", padx=10, pady=(0,10))
        self.slider_fm.configure(state="disabled")

        # Digital (placeholder)
        frm_dig = section("Modulação Digital (em breve)", "#444433")
        ctk.CTkLabel(frm_dig, text="ASK, FSK, PSK, QAM").pack(anchor="w", padx=5, pady=5)

        # Comandos
        frm_cmd = section("Comandos", "#333333")
        ctk.CTkButton(frm_cmd, text="Gerar Sinal", command=self.plot_signal).pack(fill="x", padx=10, pady=5)
        ctk.CTkButton(frm_cmd, text="Exportar CSV/JSON", command=self.export_data).pack(fill="x", padx=10, pady=5)
    ...
    def _add_entry(self, parent, label, default):
        ctk.CTkLabel(parent, text=label).pack(anchor="w", padx=5, pady=(5,0))
    ...
    def _build_plot_area(self):
        ...
        self.zoom_time = ctk.CTkSlider(ctrl, from_=0.05, to=1.0, number_of_steps=95, command=self.update_time_zoom)
        ...
        self.zoom_freq = ctk.CTkSlider(ctrl, from_=0.05, to=1.0, number_of_steps=95, command=self.update_freq_zoom)
    ...
    def _on_am_fm_toggle(self):
        # apenas uma modulação analógica por vez
        if self.mod_am.get():
    ...
    def plot_signal(self):
        ...
        if N < 128 or N % 2 != 0:
            ...
            return
        if Fc >= Fs/2:
            messagebox.showerror("Nyquist","Fc ≥ Fs/2: ajuste Fc ou Fs")
            return
        if self.mod_fm.get():
            dev = self.slider_fm.get()
            if Fc+dev >= Fs/2:
                messagebox.showerror("Nyquist","Fc+Δf ≥ Fs/2: reduza desvio FM")
                return

        t = np.arange(N)/Fs
        wf = self.waveform.get()
        # gerando portadora
        if   wf=="Seno":    y = np.sin(2*np.pi*Fc*t)
        elif wf=="Cosseno": y = np.cos(2*np.pi*Fc*t)
        elif wf=="Quadrada":y = np.sign(np.sin(2*np.pi*Fc*t))
        elif wf=="Triangular":y = 2*np.abs(2*((t*Fc)%1)-1)-1
        elif wf=="Dente de Serra": y = 2*((t*Fc)%1)-1
        elif wf=="Pulso":   y = np.where((t*Fc)%1<0.1,1,0)
        elif wf=="Ruído Branco": y = np.random.normal(0,1,N)
        elif wf=="Exp Decaimento": y = np.exp(-t)*np.sin(2*np.pi*Fc*t)
        else: y = np.sin(2*np.pi*Fc*t)

        # AM
        if self.mod_am.get():
            m = self.slider_am.get()
            y *= (1 + m * np.sin(2*np.pi*5*t))  # modulante simples 5Hz

        # FM
        if self.mod_fm.get():
            d = self.slider_fm.get()
            y = np.sin(2*np.pi*Fc*t + d*np.sin(2*np.pi*5*t))
        ...
        # salvar último
        self.last_t = t; self.last_y=y; self.last_f=f; self.last_Y=np.abs(Y)

        # plot
        self.ax_time.clear()
        self.ax_time.plot(t,y, color="cyan")
        self.ax_time.set_title("Domínio do Tempo"); self.ax_time.grid(True)

        self.ax_freq.clear()
        self.ax_freq.plot(f,np.abs(Y), color="orange")
        self.ax_freq.set_title("Domínio da Frequência"); self.ax_freq.grid(True)
    ...
    def reset_zoom(self):
        ...
        self.update_freq_zoom(1.0)

    def update_time_zoom(self, frac):
        if self.last_t is None: return
        N = len(self.last_t)
        show = int(N*frac)
        mid = N//2
        lo = max(0, mid-show//2)
