...
logging.basicConfig(
    level=logging.DEBUG,  # Mudamos para DEBUG para mais detalhes
        ...
        try:
            ...
            try:
                ...
                if freq_fundamental > 10000:  # Alta frequência
                    ...
                    view_duration = 10 * period
                else:
                    view_duration = total_duration * 0.5

                # Limita ao máximo da duração total
                view_duration = min(view_duration, total_duration)
            ...
            # Mostra apenas até 5x a frequência fundamental
            try:
                max_idx = np.argmax(Y)
                freq_fundamental = abs(f[max_idx])
                max_freq = min(5 * freq_fundamental, f[-1])
                self.ax_freq.set_xlim(0, max_freq)

                # Ajusta o slider de zoom
                self.zoom_freq.set(max_freq / f[-1])

                self.logger.info(f"Visualização frequência ajustada: 0 a {max_freq}Hz")
            except:
                # Fallback: mostra todo o espectro
                self.ax_freq.set_xlim(f[0], f[-1])
                self.zoom_freq.set(1.0)
